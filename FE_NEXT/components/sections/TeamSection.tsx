...
import { TeamMember, TeamMembersResponse } from '@/types/index';
...
export default function TeamSection(): JSX.Element {
  ...
  // Fetch team members using SWR, with locale parameter
  const { data, error, isLoading } = useSWR<TeamMembersResponse>(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/team-members?populate=*&locale=${locale}`,
    fetcher
  );
  
  const scrollLeft = (): void => {
    if (sliderRef.current) {
      sliderRef.current.scrollBy({ left: isRTL ? 300 : -300, behavior: 'smooth' });
    }
  };
    ...
    <section className="py-16 bg-[#F3F3F3]">
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <h2 className="text-3xl font-bold text-[#643F2E] mb-2">
            {t('title')}
          </h2>
