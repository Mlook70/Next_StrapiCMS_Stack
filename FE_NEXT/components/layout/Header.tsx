  ...
  return (
    <>
      <header className={cn(
        ...
        (!isHome || isScrolled || isServicesOpen) 
          ? 'bg-[#4B2615] shadow-lg' 
          : 'bg-transparent',
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between md:justify-center">
            ...
            <Link 
              href="/" 
              className={cn(
                "text-white text-xl font-bold flex items-center md:absolute md:left-4",
            ...
            <nav className={cn(
              ...
              isRTL ? "space-x-reverse space-x-8" : "space-x-5"
            )}>
              <Link href="/" className="text-white hover:text-gray-300 transition-colors px-2">
                {t('nav.home')}
              </Link>
              <Link href="/search" className="text-white hover:text-gray-300 transition-colors px-2">
                {t('nav.about')}
              </Link>
              <div className="relative group services-dropdown px-2">
                <button 
                  ...
                  className={cn(
                    "flex items-center text-white hover:text-gray-300 transition-colors",
                    isRTL ? "flex-row-reverse" : ""
                  )}
                >
                  {t('nav.services')} 
                  <ChevronDown className={cn("h-4 w-4", isRTL ? "mr-1" : "ml-1")} />
                </button>
                {isServicesOpen && (
                    ...
                    {isLoading ? (
                      ...
                      <>
                        ...
                        <div className="mt-6 text-center">
                          <Link 
                            href="/services" 
                            className="inline-block text-gray-300 hover:text-white transition-colors border border-gray-300 px-6 py-2 rounded"
                          >
                            {t('services.readMore')}
                          </Link>
                        </div>
              ...
              <Link href="/team" className="text-white hover:text-gray-300 transition-colors px-2">
                {t('nav.team')}
              </Link>
              <Link href="/search" className="text-white hover:text-gray-300 transition-colors px-2">
                {t('nav.blog')}
              </Link>
              <Link href="/contact" className="text-white hover:text-gray-300 transition-colors px-2">
                {t('nav.contact')}
              </Link>
            </nav>
            
            <div className={cn(
              "flex items-center gap-4 md:absolute md:right-4",
              ...
              {!isSearchOpen && (
                <button 
                  ...
                  className="text-white hover:text-gray-300 transition-colors px-1"
              ...
              <button 
                ...
                className="hidden md:inline-block text-white border border-white hover:bg-white hover:text-[#4B2615] transition-colors px-6 py-2 rounded"
              >
                {t('nav.appointment')}
              </button>
            ...
            <div className="md:hidden mt-4 py-4 px-2 bg-[#4B2615] rounded shadow-lg">
              <nav className="flex flex-col space-y-4">
                ...
                <button 
                  onClick={toggleServices} 
                  className={cn(
                    "flex items-center justify-between text-white hover:text-gray-300 transition-colors",
                    isRTL ? "flex-row-reverse" : ""
                  )}
                >
                ...
                <button 
                  ...
                  className="bg-white text-[#4B2615] hover:bg-gray-200 transition-colors px-4 py-2 rounded text-center"
                >
                  {t('nav.appointment')}
                </button>
                
                {/* Mobile Language Switcher - Added here too */}
                <div className="pt-2">
                  <LanguageSwitcher />
                </div>
              </nav>
